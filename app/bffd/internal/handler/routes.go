...
func RegisterHandlers(server *rest.Server, serverCtx *svc.ServiceContext) {
	...
	server.AddRoutes(
		[]rest.Route{
			...
			{
				Method:  http.MethodPost,
				Path:    "/user/verifyaccount/captcha",
				Handler: user.VerifyCaptchaWithAccountHandler(serverCtx),
			},
		},
		rest.WithPrefix("/api"),
	)
		...
		[]rest.Route{
			{
				...
				Handler: user.GetForgetPasswordCaptchaHandler(serverCtx),
			},
			{
				Method:  http.MethodPost,
				Path:    "/user/signup/captcha",
				Handler: user.GetSignupCaptchaHandler(serverCtx),
			},
		},
