...
var (
	...
	cacheYmirProductCartProductIdUserIdSizePrefix = "cache:ymir:productCart:productId:userId:size:"
)
...
type (
	productCartModel interface {
		...
		FindOneByProductIdUserIdSize(ctx context.Context, productId int64, userId int64, size string) (*ProductCart, error)
...
func (m *defaultProductCartModel) Delete(ctx context.Context, id int64) error {
	...
	ymirProductCartProductIdUserIdSizeKey := fmt.Sprintf("%s%v:%v:%v", cacheYmirProductCartProductIdUserIdSizePrefix, data.ProductId, data.UserId, data.Size)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, ymirProductCartIdKey, ymirProductCartProductIdUserIdSizeKey)
	return err
}
...
func (m *defaultProductCartModel) FindOneByProductIdUserIdSize(ctx context.Context, productId int64, userId int64, size string) (*ProductCart, error) {
	ymirProductCartProductIdUserIdSizeKey := fmt.Sprintf("%s%v:%v:%v", cacheYmirProductCartProductIdUserIdSizePrefix, productId, userId, size)
	var resp ProductCart
	err := m.QueryRowIndexCtx(ctx, &resp, ymirProductCartProductIdUserIdSizeKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
		query := fmt.Sprintf("select %s from %s where `product_id` = ? and `user_id` = ? and `size` = ? limit 1", productCartRows, m.table)
		if err := conn.QueryRowCtx(ctx, &resp, query, productId, userId, size); err != nil {
			return nil, err
		}
		return resp.Id, nil
	}, m.queryPrimary)
...
func (m *defaultProductCartModel) Insert(ctx context.Context, data *ProductCart) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.ProductId, data.UserId, data.Amount, data.IsDeleted, data.Size)
	}, ymirProductCartIdKey, ymirProductCartProductIdUserIdSizeKey)
	return ret, err
}
...
func (m *defaultProductCartModel) Update(ctx context.Context, newData *ProductCart) error {
	...
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, newData.ProductId, newData.UserId, newData.Amount, newData.IsDeleted, newData.Size, newData.Id)
	}, ymirProductCartIdKey, ymirProductCartProductIdUserIdSizeKey)
	return err
}
