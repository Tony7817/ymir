...
type (
	...
	Product struct {
		...
		DetailImages sql.NullString `db:"detail_images"`
		Size         string         `db:"size"`
...
func (m *defaultProductModel) Insert(ctx context.Context, data *Product) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.StarId, data.Name, data.Images, data.DetailImages, data.Size, data.Tags, data.Price, data.Unit, data.SoldNum, data.Description, data.Rate, data.RateCount, data.Detail, data.InStock, data.Color)
	}, ymirProductIdKey)
	return ret, err
}
...
func (m *defaultProductModel) Update(ctx context.Context, data *Product) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.StarId, data.Name, data.Images, data.DetailImages, data.Size, data.Tags, data.Price, data.Unit, data.SoldNum, data.Description, data.Rate, data.RateCount, data.Detail, data.InStock, data.Color, data.Id)
	}, ymirProductIdKey)
	return err
}
