...
type (
	...
	Product struct {
		...
		StarAvatar  string         `db:"star_avatar"`
		Name        string         `db:"name"`
...
func (m *defaultProductModel) Insert(ctx context.Context, data *Product) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Id, data.StarId, data.StarName, data.StarAvatar, data.Name, data.Tags, data.SoldNum, data.Description, data.Rate, data.RateCount, data.Detail)
	}, ymirProductIdKey)
	return ret, err
}
...
func (m *defaultProductModel) Update(ctx context.Context, data *Product) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.StarId, data.StarName, data.StarAvatar, data.Name, data.Tags, data.SoldNum, data.Description, data.Rate, data.RateCount, data.Detail, data.Id)
	}, ymirProductIdKey)
	return err
}
